...
public class UserStateTool {
    ...
    public static boolean isLoginNow(Context context) {
        if (UserTool.getLoginState(context))
            if (isNetworkEnable(context))
                return isLoginEver(context);
        return false;
    }
    ...
    public static boolean isLoginEver(Context context) {
        // return AuthService.getInstance().isLogin();//这样并不合适
        return !UserTool.getUser(context)[0].equals("");// 已经确保不会null
    }
    ...
    public static void goToLogin(Context context, boolean show_toast) {
        ...
        Intent intent = new Intent(context, LoginTitle.class);
