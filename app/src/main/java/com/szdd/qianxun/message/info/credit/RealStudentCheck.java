...
import com.szdd.qianxun.tools.connect.ConnectFile;
...
public class RealStudentCheck extends TActivity implements OnClickListener {
	...
	// 保存图片
	private void saveBitmap(String old_img_path) {
		Bitmap bmp_temp = BitmapFactory.decodeFile(old_img_path);
		round_head.setImageBitmap(bmp_temp);// 用ImageView显示出来
		ibtn_head.setImageResource(R.drawable.shape_null);
		BitmapTool.writeToFile(bmp_temp, "jpg", icon_path);
	}
	...
	private void sendToServer() {
		...
		icon_base64 = ConnectFile.encodeFile(icon_path);

		// 上传数据到服务器
		btn_submit.setEnabled(false);
		ConnectEasy.POST(this, ServerURL.CREDIT_SDU, new ConnectListener() {
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case CODE_ALBUM_START:
			if (resultCode == RESULT_OK) {
				...
				cursor.close();
				saveBitmap(picturePath);// 处理图片
			}
			break;
		case CODE_CAMERA_START:
			if (resultCode == RESULT_OK) {
				is_add_head = true;
				saveBitmap(icon_path);// 处理图片
			}
			break;
		}
	}
