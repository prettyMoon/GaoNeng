...
/**
 * 最近会话界面的定制点(根据需要实现相应的接口来达到自定义会话列表界面)，
 * 不设置则使用openIM默认的实现
 * 调用方设置的回调，必须继承BaseAdvice 根据不同的需求实现 不同的 开放的 Advice
 * com.alibaba.mobileim.aop.pointcuts包下开放了不同的Advice.通过实现多个接口，
 * 组合成对不同的ui界面的定制
 * 这里设置了自定义会话的定制
...
//                .getConversationBody();
        ...
        if (MineConfig.USER_SYSTEM.equals(id)) ChattingUICustomSample.NEED_HIDE_BOTTOM = true;
        else ChattingUICustomSample.NEED_HIDE_BOTTOM = false;
//        Log.e("EEEEEE","1");
