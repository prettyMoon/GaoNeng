...
public class MineRequestAdapter extends BaseAdapter {
    ...
    public View getView(int i, View convertView, ViewGroup viewGroup) {
        ...
        if (url1.isEmpty()) {
            ...
            rv.image2.setVisibility(View.GONE);
            rv.image3.setVisibility(View.GONE);
            ImageLoader.getInstance().displayImage(url1, rv.image1, MainMain.mOptions);
        } else if (url3.isEmpty()) {
            setImage(rv.image1, half_width, half_width);
            setImage(rv.image2, half_width, half_width);
            rv.image3.setVisibility(View.GONE);
            ImageLoader.getInstance().displayImage(url1, rv.image1, MainMain.mOptions);
            ImageLoader.getInstance().displayImage(url2, rv.image2, MainMain.mOptions);
        } else {
            ...
            ImageLoader.getInstance().displayImage(url1, rv.image1, MainMain.mOptions);
            ImageLoader.getInstance().displayImage(url2, rv.image2, MainMain.mOptions);
            ImageLoader.getInstance().displayImage(url3, rv.image3, MainMain.mOptions);
        }
        return view;
    }
