...
public class MainTab_03_RequestDetailActivity extends Activity implements
    ...
    private void Init() throws org.json.JSONException {
        ...
        users_count_detail.setText("账号：" + ("" + jsonObj.getJSONObject("userRuquest").getJSONObject("user").getLong("username")
        ).substring(0, 3) + "****" + ("" + jsonObj.getJSONObject("userRuquest").getJSONObject("user").getLong("username")).substring(7, 11));
        ...
        switch (Rid) {
            case R.id.apply_detail:
                ...
                if (UserStateTool.isLoginNow(MainTab_03_RequestDetailActivity.this)) {
                    ...
                    showToast("欢迎登录");
                    Intent intent = new Intent(MainTab_03_RequestDetailActivity.this, LoginTitle.class);
                    startActivity(intent);
                }
