...
public class RequestPowerFragment extends RequestAllFragment {
    ...
    // 加载需求
    private AdapterForTabThree powerrequestAdapter;
    private List<Map<String, Object>> powerrequestlistItems;
    // 定位相关
    ...
    public RequestPowerFragment(BaseAdapter adapter) {
        super(adapter);
    }
    ...
    public void onListClick(int position) {
        ...
        intent.putExtra("requestId", (Integer) powerrequestlistItems.get(position).get("id"));
    ...
    public boolean onLazyLoad() {
        page = -1;
        powerrequestlistItems = new ArrayList<Map<String, Object>>();
        powerrequestAdapter = new AdapterForTabThree(getActivity(), powerrequestlistItems);
        setAdapter(powerrequestAdapter);
        updateDataFromNet(page, type);
        return true;
    }
                ...
                if (response == null) {
                    ...
                    if (page_temp == -1) {
                        if (response.equals("failed")) {
                            ...
                            MainTab_03.Analysis(getActivity(), response, powerrequestlistItems);
                            res = response;
                            updateList();
                        }
                    } else {
