...
public class RequestKnowFragment extends RequestAllFragment {
    ...
    // 加载需求
    private AdapterForTabThree knowrequestAdapter;
    private List<Map<String, Object>> knowrequestlistItems;
    // 定位相关
    ...
    public RequestKnowFragment(BaseAdapter adapter) {
        super(adapter);
    }
    ...
    public void onListClick(int position) {
        ...
        intent.putExtra("requestId", (Integer) knowrequestlistItems.get(position).get("id"));
    ...
    public boolean onLazyLoad() {
        page = -1;
        knowrequestlistItems = new ArrayList<Map<String, Object>>();
        knowrequestAdapter = new AdapterForTabThree(getActivity(), knowrequestlistItems);
        setAdapter(knowrequestAdapter);
        updateDataFromNet(page, type);
        return true;
    }
                ...
                if (response == null) {
                    ...
                    if (page_temp == -1) {
                        if (response.equals("failed")) {
                            ...
                            MainTab_03.Analysis(getActivity(), response, knowrequestlistItems);
                            res = response;
                            updateList();
                        }
                    } else {
