...
public class RequestRunFragment extends RequestAllFragment {
    ...
    // 加载需求
    private AdapterForTabThree runrequestAdapter;
    private List<Map<String, Object>> runrequestlistItems;
    // 定位相关
    ...
    public RequestRunFragment(BaseAdapter adapter) {
        super(adapter);
    }
    ...
    public void onListClick(int position) {
        ...
        intent.putExtra("requestId", (Integer) runrequestlistItems.get(position).get("id"));
    ...
    public boolean onLazyLoad() {
        page = -1;
        runrequestlistItems = new ArrayList<Map<String, Object>>();
        runrequestAdapter = new AdapterForTabThree(getActivity(), runrequestlistItems);
        setAdapter(runrequestAdapter);
        updateDataFromNet(page, type);
        return true;
    }
                ...
                if (response == null) {
                    ...
                    if (page_temp == -1) {
                        if (response.equals("failed")) {
                            ...
                            MainTab_03.Analysis(getActivity(), response, runrequestlistItems);
                            res = response;
                            updateList();
                        }
                    } else {
