...
import com.szdd.qianxun.start.login.LoginTitle;
...
/**
 * Created by Administrator on 2016/3/26 0026.0026
 */
public class ServiceDetailActivity extends Activity implements View.OnClickListener {
    //定义控件
    private Context context;
    private ImageView sexImg;
    private ImageView servicetypeicon, todaycanorderimg, loctionimg;
    private TextView nickname, nametestify, studenttestify, serviceTitle, price, changeprice, describe,
            distance, canorder, servicetype;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        loctionimg = (ImageView) findViewById(R.id.service_detail_tv_loctionimg);
    ...
    private void postDataForServiceCollect(final String serviceId, final String userId) {
        StaticMethod.POST(context,
                isCollected ? ServerURL.SERVICE_DETAIL_CANCELCOLLECTOR_URL : ServerURL.SERVICE_DETAIL_ADDCOLLECTOR_URL
                , new ConnectListener() {
                    @Override
                    public ConnectDialog showDialog(ConnectDialog dialog) {
                        return dialog;
                        ...
                        if (response.equals("failed")) {
                            ...
                            if (collectstatus == 1) {
                                ...
                                if (isCollected)
                                    ...
                                    collect.setImageResource(R.drawable.mine_homepage_collect_star);
                            }
                            if (collectstatus == 1) {
                                if (isCollected) showToast("收藏成功");
                                else showToast("取消收藏");
                                ...
                                if (isCollected) showToast("已经收藏了");
                                else showToast("已经取消收藏了");
    ...
    private void Init() throws org.json.JSONException {
        ...
        if (!exchange.equals("")) {
            ...
            changeprice.setText("我要" + exchange);
        }
        //初始化服务具体描述
                ...
                new LocationListener() {
                    ...
                    public void locationRespose(String locationName, double x, double y, float limit) {
                        try {
                            ...
                            loctionimg.setImageResource(R.drawable.location_fail);
                            distance.setText("获取距离失败");
                        }
                    }
                });
    ...
    private void InitCollector() {
        if (jsonArrayCollector.length() != 0) {
            ...
            for (int i = 0; i < jsonArrayCollector.length(); i++) {
                try {
                    jo = jsonArrayCollector.getJSONObject(i);
                    if (i < 4) {
                        collectheads.get(i).setVisibility(View.VISIBLE);
                        collectheads.get(i).setOnClickListener(this);
                        getBitmapHead(IP + jo.getString("headIcon"),
                                collectheads.get(i));
                    }
                    if (jo.getString("id").equals(InfoTool.getUserID(context))) {
                        collect.setImageResource(R.drawable.collect_star_click);
                        isCollected = true;
                    }
                } catch (JSONException e) {
        ...
        switch (Rid) {
            ...
            case R.id.service_detail_im_collect:
                if (UserStateTool.isLoginNow(context))
                    ...
                    showToast("欢迎登录");
                    intent = new Intent(context, LoginTitle.class);
