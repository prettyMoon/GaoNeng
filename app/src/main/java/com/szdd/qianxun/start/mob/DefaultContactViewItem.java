...
import com.szdd.qianxun.start.mob.layout.ContactsListviewItemLayout;
import com.szdd.qianxun.start.mob.layout.Res;

public class DefaultContactViewItem implements ContactItemMaker {

	@Override
	public View getView(final HashMap<String, Object> user, View convertView,
		...
		if (convertView == null) {
			...
			convertView = ContactsListviewItemLayout
					.create(parent.getContext());

			int resId = Res.id.iv_contact;
			viewHolder.ivContact = (AsyncImageView) convertView
					.findViewById(resId);
		...
		if (user != null) {
			...
			if (user.containsKey("fia")) {
				...
				if (TextUtils.isEmpty(dspName)) {
					viewHolder.tvContact.setText(String.valueOf(user
							.get("phone")));
				} else {
					viewHolder.tvContact.setText(dspName);
				}
				int resId = getStringRes(parent.getContext(),
						"smssdk_add_contact");
				if (resId > 0) {
				...
				if (TextUtils.isEmpty(dspName)) {
					...
					ArrayList<HashMap<String, Object>> phones = (ArrayList<HashMap<String, Object>>) user
							.get("phones");
			...
			if (user.containsKey("isnew")) {
				boolean isNew = Boolean.valueOf(String.valueOf(user
						.get("isnew")));
				if (isNew) {
			...
			String iconUrl = user.containsKey("avatar") ? (String) user
					.get("avatar") : null;
			// 设置默认头像，如果有url，就去下载
			int resId = getBitmapRes(parent.getContext(),
					"smssdk_cp_default_avatar");
			if (resId > 0) {
				viewHolder.ivContact.execute(null, resId);
			}
			if (!TextUtils.isEmpty(iconUrl)) {
				Log.w(String.valueOf(user.get("displayname"))
						+ " icon url ==>> ", iconUrl);
				...
				public void onClick(View v) {
					if (user.containsKey("fia")) {
						...
						Toast.makeText(parent.getContext(),
								String.valueOf(user), Toast.LENGTH_SHORT)
								.show();
					} else {
