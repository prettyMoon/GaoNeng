...
import com.szdd.qianxun.start.mob.layout.BackVerifyDialogLayout;
import com.szdd.qianxun.start.mob.layout.IdentifyNumPageLayout;
import com.szdd.qianxun.start.mob.layout.Res;
import com.szdd.qianxun.start.mob.layout.SendMsgDialogLayout;
...
public class IdentifyNumPage extends FakeActivity implements OnClickListener,
	...
	public void onCreate() {
		...
		if (layout != null) {
			...
			etIdentifyNum = (EditText) activity
					.findViewById(Res.id.et_put_identify);
			etIdentifyNum.addTextChangedListener(this);

			tvIdentifyNotify = (TextView) activity
					.findViewById(Res.id.tv_identify_notify);
			...
			tvUnreceiveIdentify = (TextView) activity
					.findViewById(Res.id.tv_unreceive_identify);
		...
		try {
			if (DeviceHelper.getInstance(activity).checkPermission(
			...
			if (resId > 0) {
				...
				if (layout != null) {
					...
					TextView tv_title = (TextView) dialog
							.findViewById(Res.id.tv_dialog_title);
					resId = getStringRes(activity,
			...
			public void run() {
				...
				if (result == SMSSDK.RESULT_COMPLETE) {
					...
					try {
						...
						resId = getStringRes(activity, "smssdk_error_detail_"
								+ status);
		...
		runOnUIThread(new Runnable() {
			public void run() {
				...
				if (result == SMSSDK.RESULT_COMPLETE) {
					...
					int resId = 0;
					if (status >= 400) {
						resId = getStringRes(activity, "smssdk_error_desc_"
								+ status);
					} else {
						resId = getStringRes(activity, "smssdk_network_error");
					}
					if (resId > 0) {
						Toast.makeText(activity, resId, Toast.LENGTH_SHORT)
								.show();
