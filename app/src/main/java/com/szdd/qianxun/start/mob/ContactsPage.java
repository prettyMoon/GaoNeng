...
import com.szdd.qianxun.start.mob.layout.ContactListPageLayout;
import com.szdd.qianxun.start.mob.layout.Res;

/** 联系人列表页面 */
public class ContactsPage extends FakeActivity implements OnClickListener,
		TextWatcher {
				...
				if (result == SMSSDK.RESULT_COMPLETE) {
					if (event == SMSSDK.EVENT_GET_CONTACTS) {
						...
						if (rawList == null) {
							...
							contactsInMobile = (ArrayList<HashMap<String, Object>>) rawList
									.clone();
						}
						refreshContactList();
	...
	private void refreshContactList() {
		...
		for (HashMap<String, Object> friend : friendsInApp) {
			HashMap<String, Object> contact = (HashMap<String, Object>) friend
					.remove("contact");
			if (contact != null) {
				String phone = String.valueOf(friend.get("phone"));
				if (phone != null) {
		...
		runOnUIThread(new Runnable() {
			public void run() {
				...
				adapter = new ContactsAdapter(listView, friendsInApp,
						contactsInMobile);
		...
		//
		// // 造一个“电话号码-联系人”映射表，加速查询
		// HashMap<String, HashMap<String, Object>> phone2Contact = new
		...
		//
		// // 移除本地联系人列表中，包含已加入APP的联系人
		// ArrayList<HashMap<String, Object>> tmpList = new
		...
		// HashSet<HashMap<String, Object>> contactsSet = new
		// HashSet<HashMap<String,Object>>(phone2Contact.values());
		// contactsInMobile = new
		// ArrayList<HashMap<String,Object>>(contactsSet);
		//
		// } catch (Exception e) {
		// e.printStackTrace();
		// }
